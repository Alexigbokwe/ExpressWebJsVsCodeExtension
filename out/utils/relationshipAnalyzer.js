 ...
 */
async function initializeProjectCache() {
    try {
 ...
 */
async function analyzeProjectRelationships(searchCriteria = { includeAll: true }) {
    try {
        // Initialize cache if needed
        if (!projectCache.initialized) {
            await initializeProjectCache();
 ...
 */
async function findRelevantFiles(rootPath, searchCriteria) {
 ...
 */
function getNodeTypeFromFilePath(filePath) {
 ...
 */
function analyzeRelationships(filePath, content, nodeMap) {
    const edges = [];
    const fileName = path.basename(filePath, path.extname(filePath));
