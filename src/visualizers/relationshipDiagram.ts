...
import { analyzeProjectRelationships, SearchCriteria, RelationshipData } from "../utils/relationshipAnalyzer";

/**
 * Class to manage the object relationship diagram visualization
 */
export class RelationshipDiagramProvider {
  public static readonly viewType = "expresswebjs.objectRelationshipDiagram";
  private static instance: RelationshipDiagramProvider | undefined;

  private panel: vscode.WebviewPanel | undefined;
  private context: vscode.ExtensionContext;
  private disposables: vscode.Disposable[] = [];
  private lastSearchCriteria: SearchCriteria = { includeAll: true };

  /**
   * Get the singleton instance of the RelationshipDiagramProvider
   */
  public static getInstance(context: vscode.ExtensionContext): RelationshipDiagramProvider {
   ...
   */
  private async searchAndUpdate(criteria: SearchCriteria): Promise<void> {
    if (!this.panel) {
      return;
    }
    ...
    try {
      ...
      const data = await analyzeProjectRelationships(criteria);
   ...
   */
  public async refreshDiagram(): Promise<void> {
    if (!this.panel) {
   ...
   */
  private async exportDiagram(format: "svg" | "png", data: string): Promise<void> {
    try {
      // Show save dialog
